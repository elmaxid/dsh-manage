...
| dsh-engram-recap | `dsh-engram-recap` | ✅ OK (propio, escrito y publicado en esta sesión, 2026-08-28) | Origen: pedido del usuario ("Engram no guarda solo si yo se lo digo, se pierde al cortar sesión"). Inyecta un `systemPrompt.context` dinámico (re-evaluado en CADA request del agente, a diferencia de una sección `persona` estática leída una sola vez): turno 0 recuerda llamar `mem_context`/`mem_search` antes de la primera acción sustantiva; cada `remindEvery` turnos (default 6) recuerda `mem_save` si hubo algo memory-worthy. Probado en vivo en esta misma sesión: el reminder disparó real y se cumplió (`mem_context` trajo historial real del proyecto). **No llama tools de Engram directamente** — el sandbox de plugins Host confirmado SIN `AbortController`/`crypto` disponibles (probado con `typeof AbortController`/`typeof crypto`), así que no puede driving `ctx.tools.execute()` con `AbortSignal`; usa el mismo mecanismo de esteering-por-prompt que `dsh-plan-mode` para su sección de política. Solo tiene efecto si el MCP de Engram está mounted en la sesión (ver nota de `postInstallNotes` sobre MCPs de proyecto). `plugin_vet` PASS 89/100 (WARN esperables: sin lockfile propio, sin CI — paquete nuevo sin dependencias). `test_drive` PASS dos veces: contra el path local y contra el paquete real publicado en npm (install+patch+boot+uninstall, ambos limpios). Publicado bajo la cuenta `elmaxid` (misma del repo `dsh-manage`), repo `github.com/elmaxid/dsh-manage/tree/main/plugins/dsh-engram-recap`. Turn count es un `WeakMap` en memoria por `Session`, process-local — se resetea en cada reinicio del proceso (mismo trade-off que cualquier otra contribución stateless per-request de Cordis). |
